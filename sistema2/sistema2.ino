...
bool resistenciaLigada = true;  // Inicialmente ligado
...
// Histerese
double faixaHisterese = 1; // Temperatura de tolerância para evitar liga/desliga repetido

// Flag para o início do comportamento da estufa
bool estufaIniciada = true;  // Inicia com a estufa ligada
...
void controlarRele(double temperaturaAtual) {
    ...
    // Controle do relé com histerese e comportamento inicial
    if (!estufaIniciada) {
        if (temperaturaAtual > 5.0) {
            estufaIniciada = true; // Começa a controlar após temperatura inicial
        }
    } else {
        // Histerese: Se o erro for pequeno, não muda o estado do relé
        if (pidOut > faixaHisterese) {
            if (!resistenciaLigada) {
                digitalWrite(RELE_RESISTENCIA, HIGH); // Liga o relé
                resistenciaLigada = true;
            }
        } else if (pidOut < faixaHisterese) {
            if (resistenciaLigada) {
                digitalWrite(RELE_RESISTENCIA, LOW); // Desliga o relé
